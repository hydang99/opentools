...
class CLEVRMathAnswerVerification(BaseModel):
    final_answer: int
...
class ResultScorer:
    ...
    def answer_verification(self, question, response, correct_answer):
        query_prompt = f"""
        Extract the final integer answer from this CLEVR-Math response.
        Question: {question}
        Model response: {response}

        Return only the final integer number from the response, ignoring all explanations.
        """

        response = self.llm_engine(query_prompt, response_format=CLEVRMathAnswerVerification)
        try:
            prediction = response.final_answer
            is_correct = prediction == int(correct_answer)
...
if __name__ == "__main__":
    try:
        ...
        error_results_file = os.path.join(args.result_dir, "error_results.json")
        with open(error_results_file, 'w') as f:
            json.dump(wrong_results, f, indent=4)
            print(f"\nError results saved to {error_results_file}")
