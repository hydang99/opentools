...
class Planner:
    def __init__(self, llm_engine: Any, llm_engine_name: str, toolbox_metadata: dict = None, available_tools: List = None, verbose: bool = False):
    ...
    def generate_base_response(self, question: str, image: str, max_tokens: str = 4000) -> str:
        ...
        if image_info and "image_path" in image_info:
            self.base_response = self.llm_engine.generate(question, image=image, max_tokens=max_tokens)
        else:
            self.base_response = self.llm_engine.generate(question, max_tokens=max_tokens)
        if isinstance(self.base_response, dict):
            self.base_response = self.base_response.get('text')
        else:
            self.base_response = str(self.base_response)
...
"""
        ...
        if image_info and "image_path" in image_info:
            self.query_analysis = self.llm_engine.generate(query_prompt, image=image)
        else:
            self.query_analysis = self.llm_engine.generate(query_prompt)
        if isinstance(self.query_analysis, dict):
            self.query_analysis = self.query_analysis.get('text')
        else:
            self.query_analysis = str(self.query_analysis)
        return self.query_analysis.strip()
...
- Remember to use the absolute path of the image or url which was given in the query.
...
"""
        image_info = self.get_image_info(image)
        if image_info and "image_path" in image_info:
            next_step = self.llm_engine.generate(prompt_generate_next_step, image=image)
        else:
            next_step = self.llm_engine.generate(prompt_generate_next_step)
        if isinstance(next_step, dict):
            next_step = next_step.get('text')
        else:
...
"""
        ...
        if image_info and "image_path" in image_info:
            stop_verification = self.llm_engine.generate(prompt_memory_verification, image=image)
        else:
            stop_verification = self.llm_engine.generate(prompt_memory_verification)
        if isinstance(stop_verification, dict):
            stop_verification = stop_verification.get('text')
        else:
            stop_verification = str(stop_verification)
...
"""
        ...
        if image_info and "image_path" in image_info:
            final_output = self.llm_engine.generate(prompt_generate_final_output, image=image)
        else:
