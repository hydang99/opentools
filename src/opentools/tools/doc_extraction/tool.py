# source code: https://github.com/inclusionAI/AWorld/blob/main/examples/gaia/mcp_collections/documents/msdocx.py
import json, os, time, traceback, zipfile, sys
...
class DocumentMetadata(BaseModel):
    """Metadata for extracted Word document content."""
    file_name: str
    file_size: int
    file_type: str
    absolute_path: str
...
class Doc_Extraction_Tool(BaseTool):
    # Default args for `opentools test Doc_Extraction_Tool` (uses test_file/data.json)
    DEFAULT_TEST_ARGS = {
        "tool_test": "doc_extraction",
        "file_location": "doc_extraction",
    ...
    ---------------------
    Purpose:
        A comprehensive Word document extraction tool that extracts content from Word files (.docx, .doc) with support for multiple sections, embedded media extraction (images, audio, video), headers and footers extraction. Provides detailed metadata about document structure, content types, processing engine used, and extracted media files. Supports selective content extraction, table processing, and structure analysis. Uses python-docx for robust Word processing.

    Core Capabilities:
        - Word document content extraction (.docx, .doc)
        - Embedded media extraction (images, audio, video)
        - Table content extraction and formatting
        - Document structure analysis
        - Selective content extraction
        - Comprehensive metadata collection
        - Cross-platform compatibility
        - LibreOffice integration for DOC format conversion

    Intended Use:
        Use this tool when you need to extract content from Word documents, including embedded media extraction, table content extraction, and document structure analysis.

    Limitations:
        - May not handle complex Word documents or embedded media
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            description="""A comprehensive Word document extraction tool that extracts content from Word files (.docx, .doc) with support for multiple sections, embedded media extraction (images, audio, video), headers and footers extraction. Provides detailed metadata about document structure, content types, processing engine used, and extracted media files. Supports selective content extraction, table processing, and structure analysis. Uses python-docx for robust Word processing. CAPABILITIES: Word document content extraction (.docx, .doc), embedded media extraction, table content extraction and formatting, document structure analysis, selective content extraction, comprehensive metadata collection, cross-platform compatibility, LibreOffice integration for DOC format conversion. SYNONYMS: Word document extractor, DOCX reader, document parser, Word file processor, document content extractor, Word document analyzer, document structure analyzer, embedded media extractor, table extractor, document conversion tool. EXAMPLES: 'Extract content from this Word document', 'Convert DOCX to markdown format', 'Extract tables from this report', 'Get document structure and content'.""",
            parameters={
                "type": "object",
                "properties": {
                    "file_path": {
                        "type": "string",
                        "description": "Path to the Word document file to extract content from"
                    },
                    "output_format": {
                        "type": "string",
                    ...
                    "extract_images": {
                        "type": "boolean",
                        "description": "Whether to extract and save embedded images and media from the document (default: True)"
                    },
                    "extract_tables": {
                        "type": "boolean",
                        "description": "Whether to extract table content (default: True)"
                    },
                    "extract_headers_footers": {
                        "type": "boolean",
                        "description": "Whether to extract headers and footers (default: True)"
                    },
                    "include_structure": {
                        "type": "boolean",
                        "description": "Whether to include document structure information in output (default: True)"
                    }
                },
                "required": ["file_path"],
            ...
            tags=["word_extraction", "document_processing", "docx_parser", "embedded_media", "table_extraction", "document_structure", "python_docx", "document_conversion", "content_extraction", "word_processing"],
            limitation="Some complex Word features may not be fully preserved, large documents may consume significant memory, media extraction limited to embedded content, processing time scales with document size and complexity, some advanced formatting may not be captured, DOC format support requires LibreOffice conversion",
            agent_type="File_Extraction-Agent",
            accuracy= self.find_accuracy(os.path.join(os.path.dirname(__file__), 'test_result.json')),
            demo_commands= {    
                "command": "reponse = tool.run(file_path='test.docx', output_format='markdown', extract_images=True, extract_tables=True, extract_headers_footers=True, include_structure=True)",    
                        ...
                        saved_media.append(
                            {
                                ...
                                "size_bytes": str(media_path.stat().st_size),
                            }
                        )


                    except Exception as e:
                        print(f"Failed to extract media file {media_file}: {e}")

        except Exception as e:
            print(f"Could not extract media from DOCX: {e}")
    ...
    def run(
        ...
        try:
            ...
            print(f"Processing DOCX document: {file_path.name}")
            ...
            return {"error": f"Missing dependency: {str(e)}. Please install python-docx: pip install python-docx", "success": False, "traceback": traceback.format_exc()}
        except Exception as e:
            return {"error": f"DOCX extraction failed: {str(e)}", "success": False, "error_type": "docx_extraction_failed", "traceback": traceback.format_exc()}
    ...
    def test(self, tool_test: str="doc_extraction", file_location: str="doc_extraction", result_parameter: str="result", search_type: str="exact_match"):
...
if __name__ == "__main__":
    ...
    try:
        service = Doc_Extraction_Tool()
        service.embed_tool()
        service.test(tool_test="doc_extraction", file_location="doc_extraction", result_parameter="result", search_type='exact_match')
    except Exception as e:
