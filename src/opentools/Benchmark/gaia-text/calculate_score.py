...
class ResultScorer:
    def __init__(self, llm_engine=None):
        self.llm_engine = llm_engine or ChatOpenAI(model_string="gpt-5-mini", is_multimodal=False, enable_cache=False)
    ...
    def answer_verification(self, Question, response, correct_answer):
        ...
        query_prompt = f"""
        You are a semantic answer evaluator. Compare the model's response against the correct answer focusing on SEMANTIC MEANING and ALIGNMENT, not exact string matching.
        ...
        1. **Extract the core semantic meaning** from the model response (ignore explanations, context, formatting, or additional details)
        2. **The answer is CORRECT if the semantic meaning aligns** with the correct answer:
           - Accept as correct when the model's response explains or paraphrases the correct answer, even if it provides additional numbers or the justification, as long as it clearly expresses the same key factual information.
           - Accept synonyms and equivalent expressions
           - Accept different phrasings that convey the same meaning
           - Accept numerical variations that are semantically equivalent
           - Accept case-insensitive matches for all text answers
           - Accept answers that contain the correct information even if phrased differently
           - Accept partial matches if they contain the core correct information
           - Accept answers that are more specific or detailed than the expected answer
           - Accept answers that include the correct answer with additional context
           - Accept unit differences and format variations. For example:
             * If the correct answer is "52" for "how many thousand cm", then "52000 cm" is also correct
             * Numbers with the same mathematical value but different unit expressions are equivalent
        3. **The answer is INCORRECT only if**:
           - The core semantic meaning is completely different
           - The answer is factually wrong or contradicts the correct answer
           - The answer is too vague or ambiguous when the question requires precision
           - The answer is completely unrelated to the question

        **CRITICAL**: Focus on SEMANTIC ALIGNMENT and MEANING, not exact string matching. If the response conveys the same factual information or meaning as the correct answer, even with different wording, formatting, or additional context, it should be marked as CORRECT.

        Response Format:
        <analysis>: Extract the core semantic meaning from both responses and explain why they do or don't align semantically
        <true_false>: Return "True" if the semantic meaning aligns, otherwise "False"
        """
        ...
        try:
            ...
            if correct_lower in response_lower:
                analysis = f"Fallback: Correct answer '{correct_answer}' found in response"
                true_false = True
            else:
                analysis = f"Fallback: No match found. Response: '{response}', Correct: '{correct_answer}'"
                true_false = False
    ...
    def score_results(self, results, max_workers=10):
        correct = 0
        print(f"Starting to score {len(results)} results with {max_workers} workers...")
        
        def process_single_result(pid_data):
            pid, question_data = pid_data
            try:
                # Check if required keys exist
                if "response" not in question_data:
                    print(f"Error processing PID {pid}: 'response' key not found")
                    return pid, f"Error: 'response' key not found", False
                if "correct_answer" not in question_data:
                    print(f"Error processing PID {pid}: 'correct_answer' key not found")
                    return pid, f"Error: 'correct_answer' key not found", False
                
                response = question_data["response"]
                correct_answer = question_data["correct_answer"]
                # Check if response is not None or empty
                if not response or response.strip() == "":
                    print(f"Warning: PID {pid} has empty response")
                    return pid, "Empty response", False
                
                analysis, true_false = self.answer_verification(question_data["query"], response, correct_answer)
                return pid, analysis, true_false
            except Exception as e:
                print(f"Error processing PID {pid}: {str(e)}")
                return pid, f"Error: {str(e)}", False
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm.tqdm(concurrent.futures.as_completed(futures), 
                ...
                try:
                    ...
                    # Only update if the pid exists in results
                    if pid in results:
                        results[pid].update({
                        ...
                        print(f"Warning: PID {pid} not found in results when updating")
                        
                except Exception as e:
                    print(f"Error getting result for future: {str(e)}")
                    # Try to get the exception details
                    try:
                        future_exception = future.exception()
                        if future_exception:
                            print(f"Future exception details: {future_exception}")
                    except:
                        pass
        
        print(f"Scoring completed. {correct}/{len(results)} correct.")
