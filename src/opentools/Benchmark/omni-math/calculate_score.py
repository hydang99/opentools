...
class ResultScorer:
    ...
    def answer_verification(self, response, correct_answer):
        query_prompt = f"""
        This is a math problem answer verification task. Compare the model's response against the correct answer.
        
        Model response: {response}
        Correct answer: {correct_answer}

        Evaluation rules:
        1. Extract the final answer from both responses, handling various mathematical formats:
        - Numerical values (e.g., "54", "3/4", "\\frac{1}{2}")
        - Mathematical expressions (e.g., "\\sqrt{3}", "2^n")
        - Set notation (e.g., "{1,2,3}", "(1,1,1,1)")
        - Inequalities (e.g., "x > 5")
        - Text answers (e.g., "True", "Yes")
        
        2. Consider answers equivalent if they represent the same mathematical value:
        - Different but equivalent fractions (e.g., "1/2" = "\\frac{1}{2}" = "0.5")
        - Different representations of sets/tuples with same elements
        - Equivalent mathematical expressions
        - LaTeX vs plain text notation
        - Different orderings of equivalent solutions
        
        3. Special cases:
        - For multiple solutions, all solutions must match (order independent)
        - For inequalities, both the relation and values must match
        - For text answers, compare semantic meaning
        - For parametric answers, variables and conditions must match
        - IMPORTANT: If the model's response contains the correct answer (e.g., model lists multiple solutions including the correct one, or provides a superset), mark as True
        
        4. Ignore differences in:
        - Spacing and formatting
        - LaTeX decorators (e.g., \\boxed, \\text)
        - Mathematical notation style when equivalent
        - Solution explanation or steps

        Response Format:
        <analysis>: First extract the mathematical answers, then explain the comparison.
        <true_false>: Return "True" if answers are mathematically equivalent OR if the model's response contains the correct answer, otherwise "False"
        """
...
def load_data(data_file, result_dir, response_type):
    ...
    files_to_process = [f for f in os.listdir(result_dir) 
                       if f.endswith(".json") and f.startswith("output_") and len(f.split("_")) == 2]
    
    for file in tqdm(files_to_process, desc="Loading result files"):
        try:
            ...
            # Process token usage and results (for both cases: pid in result or from filename)
...
if __name__ == "__main__":
    try:
        ...
        valid_results = {}
        invalid_pids = []
        for pid, data in tqdm(results.items(), desc="Validating results", total=total_loaded):
        ...
        # Calculate the usage of tools (this uses result_dir, not log_dir, so do it regardless)
        try:
        ...
        if os.path.exists(log_dir):
            ...
            # Update the scores with step stats
            scores.update({
