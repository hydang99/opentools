...
class AlgopuzzlevqaAnswerVerification(BaseModel):
    final_answer: bool
...
class ResultScorer:
    ...
    def eval_string_based(response_text):
        import re
        ANSWER_FIELD_PATTERN = r"(?i)Answer\s*:\s*([^\n\r]*)"
        LETTER_PATTERN = r"(?i)\b([a-f])\b"

        if not isinstance(response_text, str):
            return None

        extracted_answer = None
        for match in re.finditer(ANSWER_FIELD_PATTERN, response_text):
            remainder = match.group(1)
            letter_matches = re.findall(LETTER_PATTERN, remainder)
            if letter_matches:
                extracted_answer = letter_matches[-1].lower()
    ...
    def llm_double_check(self, question, response, correct_answer):
        """Simple LLM double-check for incorrect answers."""
        try:
            prompt = f"""Question: {question}
                    Response: {response}
                    Correct Answer: {correct_answer}
                    Does the response match the correct answer?         
                    Return only the True/False, ignoring all explanations."""
            llm_response = self.llm_engine.generate(prompt, response_format=AlgopuzzlevqaAnswerVerification)
    ...
    def score_results(self, results, max_workers=10):
        ...
        def process_single_result(pid_data):
            ...
            # Double-check with LLM if initially incorrect
            llm_corrected = true_false
            if not true_false:
                llm_correct = self.llm_double_check(query, response, correct_answer)
                if llm_correct:
                    true_false = True
                    llm_corrected = True
            
            return pid, analysis, true_false, llm_corrected
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            with tqdm(total=len(futures), desc="Scoring results", unit="result", 
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        pid, analysis, true_false, llm_corrected = future.result()
                        ...
                        if llm_corrected:
                            results[pid]["llm_double_check"] = "CORRECTED"
                        pbar.update(1)
...
def load_data(data_file, result_dir, response_type):
    ...
    with open(data_file, 'r') as f:
        ...
        # Load the results  
    results = {}
    tokens_usage = {"total_tokens": 0, "total_prompt_tokens": 0, "total_completion_tokens": 0, "call_count": 0}
            ...
            with open(os.path.join(result_dir, file), 'r') as f:
                ...
                if pid in benchmark_data:
                    ...
                    if response_type in result:
                        ...
                        alternative_types = ["final_answer", "direct_answer","direct_output", "base_response", "response","full_answer"]
...
def parse_args():
    ...
    parser.add_argument("--response_type", type=str, default="final_answer", 
                        choices=["final_output", "direct_output", "base_response", "full_answer"],
...
if __name__ == "__main__":
    try:
        ...
        # Enrich error stats with simple rates
